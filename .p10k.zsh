# Generated by Powerlevel10k configuration wizard on 2024-11-25 at 15:00 EST.
...
# 24h time, angled separators, slanted heads, flat tails, 2 lines, dotted, no frame,
...
() {
  ...
  typeset -g POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=(
    ...
    vcs                     # git status
    jobid                   # current job id for slurm, lsf, pbs
    # =========================[ Line #2 ]=========================
    newline                 # \n
  ...
  function prompt_jobid() {
      if [[ -n $LSB_JOBID ]]; then
          JOBID="LSB #$LSB_JOBID"
      fi
      if [[ -n $SLURM_JOBID ]]; then
          JOBID="SLURM #$SLURM_JOBID"
      fi
      if [[ -n $PBS_JOBID ]]; then
          JOBID="PBS #$PBS_JOB"
      fi
      p10k segment -e -f '140' -t "$JOBID"
  }

  function instant_prompt_jobid() {
      prompt_jobid
  }
